...
tissuevsULA_output <- generate_DE_results(dds, c("ULA","tissue" ))
res <-read.csv("C:\\Users\\SamGa\\Downloads\\FYP Stuff\\GSEA_Rfiles\\ULA_vs_tissue_allgenes.csv", header = T)
plot_volcano(res, 0.05, nlabel = 15, label.by = "padj","ULA vs tissue")
...
ranked_list <- read.table("C:\\Users\\SamGa\\Downloads\\FYP Stuff\\GSEA_Rfiles\\ULA_vs_tissue_rank.rnk", header = T, stringsAsFactors = F)
...
waterfall_plot(fgsea_results, "Hallmark pathways altered in ULA culture")
